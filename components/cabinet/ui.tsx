...
export function StatTile({ icon, value, label, tone }: { icon: IconName; value: ReactNode; label: string; tone?: 'good' | 'warn' | 'bad' }) {
  ...
  return (
    <div className="p-card p-4">
      ...
      <p className={'mt-2 text-2xl font-extrabold leading-none tabular-nums ' + valueColor}>{value}</p>
...
export function IconButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      title={label}
      className={'flex h-9 w-9 shrink-0 items-center justify-center rounded-full transition disabled:opacity-30 ' + color}
    >
      <Icon name={icon} size={16} />
    </button>
  );
...
export function SegmentedTabs<T extends string>({
  ...
  return (
    <div className="flex gap-1.5 overflow-x-auto rounded-full bg-[#f0f1ec] p-1">
      {items.map((item) => (
        <button
          key={item.id}
          onClick={() => onChange(item.id)}
          className={
            'p-btn shrink-0 whitespace-nowrap px-3.5 py-1.5 text-[11px] ' +
...
export function Sheet({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#1a1d1e]/60 sm:items-center sm:p-6" onClick={onClose}>
      <div
        className="p-fade-up flex max-h-[88vh] w-full flex-col rounded-t-[28px] bg-white sm:max-w-md sm:rounded-[28px]"
      ...
      >
        <div className="flex items-center gap-3 border-b border-[#e7e9e2] px-5 py-4">
          ...
          <button
            onClick={onClose}
            aria-label="Закрыть"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#f5f6f1] text-[#5c6066] transition hover:bg-[#e7e9e2]"
          >
            <Icon name="x" size={16} />
          </button>
        </div>
        ...
        {footer && <div className="flex gap-2 border-t border-[#e7e9e2] px-5 py-4">{footer}</div>}
      </div>
    </div>
  );
}
