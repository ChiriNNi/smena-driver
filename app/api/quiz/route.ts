...
// Вопросы теста по ТБ.
//
// Водителю вопросы уходят без правильного ответа — иначе тест проверяет
// только умение открыть инструменты разработчика. Администратору correct
// нужен: он его и задаёт в редакторе.
...
export async function POST(req: NextRequest) {
  return withAdmin(async () => {
    const { question, options, correct } = parseQuestion(await readBody(req));
    const row = await queryOne<QuizRow>(
      `INSERT INTO quiz_questions (question, options, correct_index, position)
       VALUES ($1, $2::text[], $3, (SELECT coalesce(max(position), -1) + 1 FROM quiz_questions))
       RETURNING id, question, options, correct_index`,
